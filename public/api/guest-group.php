...
try {
    ...
    if ($guest['mailing_group'] !== null) {
        $stmt = $pdo->prepare("
            SELECT id, first_name, last_name, group_name, mailing_group, attending, ceremony_attending, reception_attending, dietary, song_request, message, email, rsvp_submitted_at, has_plus_one, plus_one_name, plus_one_attending, plus_one_ceremony_attending, plus_one_reception_attending, plus_one_dietary
            FROM guests
            WHERE mailing_group = ?
            ORDER BY id ASC
        ");
