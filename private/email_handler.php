...
function sendEmail($to, $subject, $body, $replyTo = null, array $options = []) {
    try {
        ...
        // Use verified sending domain from Resend
        // SMTP_FROM_EMAIL should be set to a verified domain email in Resend
        // This is required to avoid "unsigned" rejections
        $fromEmail = $_ENV['SMTP_FROM_EMAIL'] ?? $_ENV['FROM_EMAIL'] ?? null;

        if (empty($fromEmail)) {
            // Fallback: try to construct from verified domain if SMTP_VERIFIED_DOMAIN is set
            $verifiedDomain = $_ENV['SMTP_VERIFIED_DOMAIN'] ?? null;
            if ($verifiedDomain) {
                ...
                error_log("Warning: Using unverified FROM email address. Set SMTP_FROM_EMAIL or SMTP_VERIFIED_DOMAIN in .env to use a verified domain.");
            }
        }
